...
Write-Step "Creating grouped assisted Monday mapping review workbook only. No database writes, no imports, no provider calls."
...
if (-not (Test-Path -LiteralPath $workbookFile)) {
    throw "Assisted workbook was not created: $workbookFile"
}

Write-Host ""
Write-Host "Assisted Monday mapping review workbook"
